...
def compare(famList):
    ...
    orb2 = selectOrbian(famList, orb1) # Remove this line to work on this function. It is here to prevent errors until it's defined
...
def toPasture(famList):
    ...
    orb1 = selectOrbian(famList)
    orb1Index = famList.index(orb1)
    famList.pop(orb1Index)
    print("Farewell to our dear Orbian Friend " + orb1.getName())
...
def thanosSnap(famList):
    ...
    halfLengthOfFamlist = int(len(famList)/2)
    shuffle(famList)
    for i in range(halfLengthOfFamlist):
        famList.pop()
